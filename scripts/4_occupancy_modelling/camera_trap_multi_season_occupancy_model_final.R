#probability of detection for camera trap using occupancy modelling
...
df_prep_ct <- df %>%
  ...
  rename(ct = count) %>%
  ungroup()%>% 
  mutate(ct_binary = case_when(ct > 0 ~ 1,
                                       TRUE ~ 0))
...
completed_survey <- expanded_dates %>%
  left_join(df_prep_ct, by = c("site", "week")) %>%
  mutate(ct_binary = ifelse(is.na(ct_binary), 0, ct_binary))

# --- Create Weekly Presence/Absence Matrix ---
presence_weekly <- completed_survey %>%
  dplyr::select(site, week, ct_binary) %>%
  pivot_wider(names_from = week, values_from = ct_binary, id_cols = site, values_fill = NA) %>%
  as.data.frame()
...
prep_weekly_temperature <- temperature %>%
  mutate(week = week(date),
         season = case_when(week > 45 & week <= 53 | week >= 1 & week <= 3 ~ "wet",
                            week > 3 & week <= 12 ~ "late_wet",
                            week > 12 & week <= 25 ~ "early_dry",
...
weekly_temp <- prep_weekly_temperature %>%
...
prep_weekly_season <- expanded_dates %>%
...
weekly_season <- as.data.frame(prep_weekly_season)
...
# models1 <- list(
#   m1 = fm0, f1 = fixed1, f2 = fixed2, f3 = fixed3, f4 = fixed4, f5 = fixed5, f6 = fixed6, f7 = fixed7, f8 = fixed8, f9 = fixed9,
#   f10 = fixed10, f11 = fixed11, f12 = fixed12, f13 = fixed13, f14 = fixed14, f15 = fixed15, f16 = fixed16, f17 = fixed17,
#   f18 = fixed18, f19 = fixed19, f20 = fixed20, f21 = fixed21, f22 = fixed22, f23 = fixed23, f24 = fixed24, f25 = fixed25,
#   f26 = fixed26, f27 = fixed27, f28 = fixed28, f29 = fixed29, f30 = fixed30, f31 = fixed31, ff = fixed_full
# )

# # --- Function to Calculate QAIC for a List of Models ---
...
# 
#     mb.boot <- AICcmodavg::mb.gof.test(model, nsim = 1000, plot.hist = FALSE, plot.seasons = T)
...
models1 <- list(
  ...
  f2 = fixed2, 
  f3 = fixed3, 
  f4 = fixed4, f5 = fixed5, f6 = fixed6, f7 = fixed7, f8 = fixed8, f9 = fixed9,
  f10 = fixed10, f11 = fixed11, 
  f12 = fixed12, f13 = fixed13, f14 = fixed14, 
  f15 = fixed15, f16 = fixed16, f17 = fixed17,
  f18 = fixed18, f19 = fixed19, f20 = fixed20, f21 = fixed21, 
  f22 = fixed22, f23 = fixed23, f24 = fixed24, 
  f25 = fixed25,
...
# --- Calculate QAIC and Print Results ---
# qaic_results <- calculate_qaic(models1)
# print(qaic_results)

AICcmodavg::mb.gof.test(fixed_full, nsim = 1000, plot.seasons = T, print.table = T, plot.hist = T)

AICcmodavg::aictab(models1, c.hat = 1) #QAIC suggests models f25, f21, f24, f23, f20 and f18

unmarked::vif(fixed20, type = 'det')
unmarked::vif(fixed25, type = 'det')
unmarked::vif(fixed21, type = 'det')
unmarked::vif(fixed24, type = 'det')
unmarked::vif(fixed23, type = 'det')
unmarked::vif(fixed18, type = 'det')

list <- fitList(f20 = fixed20, f21 = fixed21, f18 = fixed18, f23 = fixed23, f24 = fixed24, f25 = fixed25)
...
# Combine seasonal simulations
# sim_season <- sim_temp %>% dplyr::select(season_weekly) %>% 
#   mutate(temp_weekly = 1,
#   removal_num_weekly = 1,
# variable = "temp_weekly",
#%>% 
# pivot_longer(cols = 1:5, names_to = "season_weekly", values_to = "removal_num_weekly")

# sim_season2 <- data.frame(
#   dry = runif(25, min = rain_mean$q1[2], max = rain_mean$q2[1]),
#   late_dry = runif(25, min = rain_mean$q1[3], max = rain_mean$q2[3]),
#   wet = runif(25, min = rain_mean$q1[4], max = rain_mean$q2[4]),
#   late_wet = runif(25, min = rain_mean$q1[5], max = rain_mean$q2[5]),
#   variable = "rain_weekly",
#   index = 1:100
# )

# sim_all <- full_join(sim_removal, sim_temp) #%>%
# pivot_longer(cols = 1:5, names_to = "season_weekly", values_to = "values") %>%
# pivot_wider(names_from = variable, values_from = temp_weekly) %>%
# dplyr::select(-index, -values) %>%
# distinct()

# Predict detection probabilities
# sim_rain$det <- predict(fixed25, type = 'det', newdata = sim_rain)

# sim_removal$det <- predict(fixed24, type = 'det', newdata = sim_removal)
# sim_season$det <- predict(fixed15, type = 'det', newdata = sim_season)
...
save_csv(sim_all, "results/image_predicted_results.csv")
# save_csv(sim_removal, "results/simulated_sound_removal_num_results.csv")
# save_csv(sim_season, "results/simulated_sound_season_results.csv")
