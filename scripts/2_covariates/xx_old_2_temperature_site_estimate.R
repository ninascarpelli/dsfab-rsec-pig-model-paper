...
yourka_data <- load_csv("pig_model_paper/data/raw_weather_measured.csv")


# Load and clean SILO climate data
silo <- load_csv("pig_model_paper/data/processed_silo_all_data.csv") %>%
...
start_date <- min(yourka_data$date, na.rm = TRUE)
end_date <- max(yourka_data$date, na.rm = TRUE)
...
save_csv(completed_dataset, "pig_model_paper/data/processed_composite_climatic.csv")
